...
The same is true for our embedded invisible interfaces where the perceived affordances will most likely be tied to the object in which the interface is embedded and not necessarily the interface itself.
So for such objects we need a more exploratory approach where the affordances are learned through use and feedback from the object, moving away from Gibson's focus on what can be \emph{seen}. 
