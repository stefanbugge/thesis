%!TEX root = ../thesis.tex
This section will focus on describing and discussing a selection of concepts that we have made for ad hoc interfaces based on the jamming technique.
...
The idea of composition is quite similar to that of computer graphics where polygons are used to compose images that appear three-dimensional.
...
Due to our issues with jamming, this was again a setup with a vacuum cleaner, a plastic bag as the membrane, and ground coffee as the particles, see figure \ref{fig:ch:jamming:concepts:inputs-prototype}.
The prototype shows how a flat and solid-like surface can change to feature different concave shapes which could function as input controls.
The tactile perception is the same on the entire surface and a cell-based jamming approach would be required to make distinct tactile areas.
...
\end{subfigure}
\caption[A primitive exploratory prototype with basic jamming of a single volume]
{A primitive exploratory prototype with basic jamming of a single volume. Ground coffee is enclosed withing a transparent plastic bag. Wrinkles occur due to the plastic bag not being very flexible.}
...
\begin{figure}[h]
  ...
  \end{subfigure}
  \caption[An exploratory prototype of playful blobs]
  {An exploratory prototype of playful blobs. Created with a rubber latex glove, ground coffee, and a vacuum cleaner. For a video of the above steps see appendix~\ref{app:videos:jamming} which also shows the rigidity of the shape when jammed.}
...
\nameref{ch:jamming:related-work:hci:claytric} from the related work section showed a rather large surface area which could be sculpted and still appear flat in its original state.
Applying this to a wall surface could be an interesting deployment.
...
\begin{figure}[h]
  ...
  \end{subfigure}
  \caption[Sketches of direct deformations of a wall by pushing and pulling]
  {Sketches of direct deformations of a wall by pushing and pulling, e.g. an improvised shelf.}
